...
let MEMORI_IURAN_GLOBAL = {}; // Memori penyimpan arsip iuran dari Firebase
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Set default select bulan & tahun pada matriks kalender
...
async function muatSistemWarga() {
    ...
    try {
        ...
        // Trigger render ulang kalender setelah memori warga terisi
        renderBukuKalender();
    } catch (e) { 
...
// ARSIP BERUBAH MENJADI LOGIKA REDIRECT DATA INSTAN KE FILTER MATRIKS
...
// LOGIKA UTAMA MATRIKS KALENDER IURAN BULANAN & HARIAN
function renderBukuKalender() {
    ...
    // Menghitung jumlah hari sesungguhnya dalam 1 bulan penuh
    ...
    // 1. RENDER HEADER KOLOM KANAN (Nama Anggota | 1 | 2 | 3 | ... | Total)
    ...
    // 2. LOOPS GENERATOR BARIS WARGA & CHECKPOINT TANGGAL KAS
    let bodyHTML = '';
    ...
    listWargaKeys.forEach(wKey => {
        ...
        // Nama Warga menggunakan "sticky left-0" agar saat discroll ke kanan namanya tidak ikut hilang tergeser
        ...
        // Looping untuk mencocokkan nominal di setiap tanggal
        for (let tanggal = 1; tanggal <= jumlahHari; tanggal++) {
            ...
            // Cari data iuran warga ini berdasarkan warga_key & kecocokan tanggal bayar
            ...
            if (totalBayarHariIni > 0) {
                ...
                // Mengonversi nominal besar menjadi format ringkas ribuan (misal 20000 -> 20k) supaya hemat ruang kolom
                const nominalRingkas = totalBayarHariIni >= 1000 ? `${Math.floor(totalBayarHariIni / 1000)}k` : totalBayarHariIni;
                bodyHTML += `<td class="p-1 text-center font-black text-emerald-600 bg-emerald-50/30 border-r border-slate-100/60 text-[9px]">${nominalRingkas}</td>`;
            } else {
                bodyHTML += `<td class="p-1 text-center text-slate-300 border-r border-slate-100/40 font-normal">-</td>`;
            }
        }
