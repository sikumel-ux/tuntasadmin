...
function pilihStatusSampah(statusCode) {
    ...
    const el = document.getElementById(`status-${statusCode}`);
...
// FUNGSI MEMUAT DAN MERENDER WARGA (TOLERAN EROR & LENGKAP DENGAN AKSES EDIT & HAPUS)
async function muatSistemWarga() {
    try {
        ...
        if(!data || Object.keys(data).length === 0) {
            list.innerHTML = `<div class="p-4 text-center text-xs text-slate-400 font-bold uppercase">Belum ada data warga aktif.</div>`;
            return;
        }
        ...
        Object.keys(data).forEach(key => {
            ...
            const tipeLabel = (w.tipe || w.Tipe || 'tetap').toUpperCase();
            const usernameWa = w.username || w.Hp || w.hp || '-';
            const gabungBulan = w.bulan_bergabung || w.bulan || '-';
        ...
        document.getElementById('listWarga').innerHTML = `<div class="p-4 text-center text-xs text-rose-600 font-bold uppercase">Gagal memuat koneksi database!</div>`;
    }
}
...
function simpanWarga(e) {
    ...
    fetch(`${DB_URL}/warga_rt04.json`, { method: 'POST', body: JSON.stringify(body) }).then(() => { 
        document.getElementById('formWarga').reset(); 
        showNotif('Warga Berhasil Didaftarkan', 'sukses'); 
        muatSistemWarga(); 
    });
}
...
function perbaruiWarga(e) {
    ...
    const body = {
        ...
        foto: (MEMORI_WARGA_GLOBAL[key] && MEMORI_WARGA_GLOBAL[key].foto) ? MEMORI_WARGA_GLOBAL[key].foto : "default.png"
    };

    document.getElementById('loadingOverlay').style.display = 'flex';
    
    fetch(`${DB_URL}/warga_rt04/${key}.json`, { 
        method: 'PUT', 
    ...
    .then(() => {
        ...
        showNotif('Profil Warga Berhasil Diperbarui', 'sukses');
        muatSistemWarga();
    })
    .catch(err => {
        console.error(err);
...
function hapusWarga(key) {
    ...
    ACTION_HAPUS_CALLBACK = () => {
        fetch(`${DB_URL}/warga_rt04/${key}.json`, { method: 'DELETE' }).then(() => { 
