...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Set default tanggal hari ini untuk input kalender Kas Baru
...
// MEMUAT SISTEM WARGA DENGAN AKSI EDIT & HAPUS
async function muatSistemWarga() {
    try {
        ...
        if(!data) {
            list.innerHTML = `<div class="p-4 text-center text-xs text-slate-400 font-bold uppercase">Belum ada data warga.</div>`;
            return;
        }
        ...
        Object.keys(data).forEach(key => {
            ...
            list.insertAdjacentHTML('beforeend', `
                <div class="p-3.5 flex justify-between items-center bg-white hover:bg-slate-50 transition-colors">
                    <div class="space-y-1">
                        <div class="flex items-center gap-2">
                            <p class="font-extrabold text-slate-800 uppercase tracking-wide text-xs">${w.nama}</p>
                        ...
                        <p class="text-[10px] text-slate-400 font-semibold">WA: ${w.username} | Pass: ${w.password || '-'}</p>
                        <p class="text-[9px] text-slate-500 font-medium">Gabung: ${w.bulan_bergabung || '-'}</p>
                    </div>
                    <div class="flex gap-1.5">
                        <button onclick="bukaModalEditWarga('${key}')" class="w-8 h-8 rounded-lg bg-teal-50 text-teal-700 flex items-center justify-center hover:bg-teal-100 transition-all active:scale-95">
                            <i class="fa-solid fa-pen-to-square text-xs"></i>
                        </button>
                        <button onclick="hapusWarga('${key}')" class="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-all active:scale-95">
...
// BUKA MODAL EDIT WARGA & ISI VALUE DATA LAMA
function bukaModalEditWarga(key) {
    const warga = MEMORI_WARGA_GLOBAL[key];
    if (!warga) return;

    document.getElementById('editWargaId').value = key;
    document.getElementById('editTipe').value = warga.tipe || 'tetap';
    document.getElementById('editNama').value = warga.nama;
    document.getElementById('editHp').value = warga.username;
    document.getElementById('editPass').value = warga.password || '';
    document.getElementById('editBulan').value = warga.bulan_bergabung || '';

    openModal('mEditWarga');
}

// PUT / UPDATE PROSES DATA WARGA KE DATABASE
function perbaruiWarga(e) {
    e.preventDefault();
    const key = document.getElementById('editWargaId').value;
    
    const body = {
        tipe: document.getElementById('editTipe').value,
        nama: document.getElementById('editNama').value.trim().toUpperCase(),
        username: document.getElementById('editHp').value.trim(),
        password: document.getElementById('editPass').value.trim(),
        bulan_bergabung: document.getElementById('editBulan').value.trim(),
        foto: MEMORI_WARGA_GLOBAL[key].foto || "default.png"
    };

    document.getElementById('loadingOverlay').style.display = 'flex';
    fetch(`${DB_URL}/warga_rt04/${key}.json`, { method: 'PUT', body: JSON.stringify(body) })
        .then(() => {
            closeModal('mEditWarga');
            document.getElementById('formEditWarga').reset();
            showNotif('Data Warga Berhasil Diperbarui', 'sukses');
            muatSistemWarga();
        })
        .catch(err => {
            console.error(err);
            showNotif('Gagal memperbarui data', 'gagal');
        })
        .finally(() => {
