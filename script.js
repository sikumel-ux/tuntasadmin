...
// EDIT: Sekarang menangkap tanggal custom dari input kalender kas
function simpanKasUmum(e) {
    ...
    fetch(`${DB_URL}/kas_rt04.json`, { method: 'POST', body: JSON.stringify(body) }).then(() => { 
        ...
        // Reset default tanggal kas hari ini
        if(document.getElementById('kasTgl')) {
...
// EDIT: Otomatis mendeteksi nomor WhatsApp warga untuk kirim pesan langsung
function simpanIuran(e) {
    ...
    // Mengambil profil warga global berdasarkan key untuk mendapatkan nomor HP
    const dataWargaObj = MEMORI_WARGA_GLOBAL[wargaKey] || {};
    let nomorHpWarga = dataWargaObj.username || ""; // Kolom username digunakan untuk menampung nomor WA

    // Format nomor WhatsApp supaya valid (mengubah prefix 08 menjadi 62)
    if (nomorHpWarga.startsWith('0')) {
        nomorHpWarga = '62' + nomorHpWarga.slice(1);
    }
    // Bersihkan karakter non-digit jika tidak sengaja terinput
    ...
    fetch(`${DB_URL}/iuran_sampah.json`, { method: 'POST', body: JSON.stringify(body) }).then(() => {
        ...
        // EDIT: Inject nomor WA tujuan langsung ke API WhatsApp link
